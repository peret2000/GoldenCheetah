...
Perspective::Perspective(Context *context, const QString& title, const QString& viewName) :
    GcWindow(context), context(context), active(false),  resizing(false), clicked(NULL), dropPending(false),
    viewName_(viewName), title_(title), chartCursor(-2), df(NULL), expression_(""), trainswitch(None)
{
    SSS;
    // setup control area
...
Perspective::importChart(const QMap<QString,QString>& properties, bool select)
{
    ...
    chart->setProperty("view", static_cast<std::underlying_type_t<GcViewType>>(viewType()));
...
Perspective::addChart(GcChartWindow* newone)
{
    ...
    if (newone) {
        ...
        RideItem *notconst = (RideItem*)context->currentRideItem();
        newone->setProperty("view", static_cast<std::underlying_type_t<GcViewType>>(viewType()));
...
ImportChartDialog::importClicked()
{
    ...
    for(int i=0; i<list.count(); i++) {
        ...
        if (static_cast<QCheckBox*>(table->cellWidget(i,0))->isChecked()) {
            ...
            if (view == tr("Equipment"))   { viewType=GcViewType::VIEW_EQUIPMENT; context->mainWindow->selectEquipment(); }
